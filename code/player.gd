...
func _mouvement_loop():
	...
	if Input.is_action_just_pressed("dash") and can_dashing:
		can_dashing = false
		dashing = true
		yield(get_tree().create_timer(0.5),'timeout')
		can_dashing = true
	input_vector.x = Input.get_action_strength("droite") - Input.get_action_strength("gauche")
	input_vector.y = Input.get_action_strength("bas") - Input.get_action_strength("haut")
	...
	if input_vector.x != 0 or input_vector.y != 0 and !mort:
		$Anim.play("walk")
